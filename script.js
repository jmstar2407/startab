...
async function cambiarCategoria(categoriaId) {
    ...
    await aplicarFondoCategoria(categoriaId);
    await renderizarIconos();
}
...
function aplicarFondoConFade(fondoConfig) {
    ...
    // Obtener el fondo actual si existe
    let fondoActual = document.getElementById('fondo-activo');

    // SIEMPRE usar fade - sin importar si el tipo es igual o diferente
    // Esto asegura que el efecto fade funcione para TODOS los cambios:
    // - Video → Imagen
    // - Video → Degradado
    // - Video → Video
    // - Imagen → Imagen
