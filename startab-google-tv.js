(() => {
  ...
  const state = {
    ...
    brightnessTimer: 0, pendingBrightness: null, optimisticBrightness: null,
          ...
          <div class="startab-tv-body startab-tv-remote-body">
            ...
            <div class="startab-tv-remote-content" id="startab-tv-remote-content">
              <div class="startab-tv-navigation-stage">
                ...
                <div class="startab-tv-dpad" aria-label="Cruceta de navegación">
                  <button class="startab-tv-dpad-btn is-up" id="startab-tv-dpad-up" type="button" data-tv-control aria-label="Arriba"><svg viewBox="0 0 24 24"><path d="m6 14 6-6 6 6"></path></svg></button>
                  <button class="startab-tv-dpad-btn is-left" id="startab-tv-dpad-left" type="button" data-tv-control aria-label="Izquierda"><svg viewBox="0 0 24 24"><path d="m14 6-6 6 6 6"></path></svg></button>
                  <button class="startab-tv-dpad-ok" id="startab-tv-ok" type="button" data-tv-control>OK</button>
                  <button class="startab-tv-dpad-btn is-right" id="startab-tv-dpad-right" type="button" data-tv-control aria-label="Derecha"><svg viewBox="0 0 24 24"><path d="m10 6 6 6-6 6"></path></svg></button>
                  <button class="startab-tv-dpad-btn is-down" id="startab-tv-dpad-down" type="button" data-tv-control aria-label="Abajo"><svg viewBox="0 0 24 24"><path d="m6 10 6 6 6-6"></path></svg></button>
                </div>
              ...
              <div class="startab-tv-app-row">
                <button id="startab-tv-youtube" class="is-youtube" type="button" data-tv-control><span class="app-mark">▶</span><b>YouTube</b></button>
                <button id="startab-tv-netflix" class="is-netflix" type="button" data-tv-control><span class="app-mark">N</span><b>Netflix</b></button>
                <button id="startab-tv-xuper" class="is-xuper" type="button" data-tv-control><span class="app-mark">X</span><b>XuperTV</b></button>
              </div>
  ...
  function cacheDom() {
    const ids = [
      ...
      'power','settings','dpad-up','dpad-down','dpad-left','dpad-right','ok','back','home','cursor','youtube','netflix','xuper',
      ...
      'touchpad-layer','touchpad-close','touch','scroll',
  ...
  function applyRemoteState(data = {}) {
    if (Number.isFinite(Number(data.volume))) state.optimisticVolume = clamp(Number(data.volume), 0, 100);
    if (typeof data.muted === 'boolean') state.muted = data.muted;
    if (typeof data.powerOn === 'boolean') state.powerOn = data.powerOn;
    if (Number.isFinite(Number(data.brightnessLevel))) state.optimisticBrightness = clamp(Math.round(Number(data.brightnessLevel)), 0, 10);
  }

  function render() {
    const d = selectedDevice();
    if (d) {
      if (state.optimisticVolume == null && Number.isFinite(Number(d.volume))) state.optimisticVolume = clamp(Number(d.volume), 0, 100);
      if (state.optimisticBrightness == null && Number.isFinite(Number(d.brightnessLevel))) state.optimisticBrightness = clamp(Math.round(Number(d.brightnessLevel)), 0, 10);
  ...
  function connectSelectedLocal() {
    ...
    ws.onmessage = ev => {
      try {
        ...
        if (data.type === 'state' && data.ok) { state.wsReady = true; state.wsConnecting = false; applyRemoteState(data); render(); }
        if (data.type === 'ack') { applyRemoteState(data); render(); }
      } catch (_) {}
    };
    ws.onerror = () => {};
  ...
  function setVolume(value) {
    value=Math.round(clamp(value,0,100)); state.optimisticVolume=value; if(dom.volume)dom.volume.value=String(value); if(dom.volumeValue)dom.volumeValue.textContent=`${value}%`; if(dom.volumeFill)dom.volumeFill.style.setProperty('--tv-volume',`${value}%`);
  ...
  function setBrightness(level) {
    level = Math.round(clamp(level,0,10)); state.optimisticBrightness = level; render();
    if (wsSend({t:'brightness', level})) return;
    state.pendingBrightness = level; clearTimeout(state.brightnessTimer);
    state.brightnessTimer = setTimeout(()=>{ const v=state.pendingBrightness; state.pendingBrightness=null; firebaseMerge({ actionCommand:{id:unique(), type:'brightness', level:v, clientAt:Date.now()} }); }, 90);
  }
  ...
  function bindUi() {
    ...
    dom.toggle?.addEventListener('click',()=>{ dom.modal?.classList.add('is-open'); dom.modal?.setAttribute('aria-hidden','false'); document.documentElement.classList.add('startab-tv-modal-open'); connectSelectedLocal(); render(); });
    const close=()=>{ stopQrScanner(); closeAddModal(); closeTouchpad(); dom.modal?.classList.remove('is-open'); dom.modal?.setAttribute('aria-hidden','true'); document.documentElement.classList.remove('startab-tv-modal-open'); };
    ...
    dom.deviceSelect?.addEventListener('change',()=>{state.selectedId=dom.deviceSelect.value||'';localStorage.setItem(SELECTED_KEY,state.selectedId);state.optimisticVolume=null;state.optimisticBrightness=null;closeWs();connectSelectedLocal();render();});
    ...
    dom.youtube?.addEventListener('click',()=>sendAction('launchApp',{app:'youtube'}));
    dom.netflix?.addEventListener('click',()=>sendAction('launchApp',{app:'netflix'}));
    dom.xuper?.addEventListener('click',()=>sendAction('launchApp',{app:'xuper'}));
