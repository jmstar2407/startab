...
(() => {
  ...
  function connectNative() {
    ...
    nativePort.onMessage.addListener((message) => {
      ...
      if (message.type === 'commandResult' && message.requestId) finishNativeReply(message.requestId, message);
    ...
    nativePort.onDisconnect.addListener(() => {
      for (const id of nativeReplies.keys()) finishNativeReply(id,{ok:false,reason:"native-disconnected"});
  ...
  function finishNativeReply(id, result) {
    const pending = nativeReplies.get(id);
    if (!pending) return;
    nativeReplies.delete(id); clearTimeout(pending.timer); pending.resolve(result);
  }
  function sendNativeAndWait(command) {
    const requestId = crypto.randomUUID();
    return new Promise(resolve => {
      const timer = setTimeout(() => finishNativeReply(requestId, {ok:false, reason:'native-timeout'}), 2200);
      nativeReplies.set(requestId, {resolve,timer});
      if (!postNativeCommand({...command,requestId})) finishNativeReply(requestId,{ok:false,reason:'native-disconnected'});
    });
  }
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.type === 'STARTAB_WINDOWS_NATIVE_COMMAND') {
      if (message.awaitResult && nativeState?.commandAcknowledgements === true) {
        sendNativeAndWait(message.command).then(sendResponse);
        return true;
      }
