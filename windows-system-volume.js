(() => {
  ...
  function renderDevices() {
    if (state.native.connected && state.native.deviceId) {
      const id = state.native.deviceId;
      state.devices.set(id, {...state.devices.get(id), deviceId:id, deviceName:state.native.deviceName,
        volume:state.native.volume, muted:state.native.muted, online:true, clientAt:Date.now()});
    }
  ...
  function applyNativeStatus(payload) {
    ...
    if (state.native.deviceId && state.native.connected) {
      const id = state.native.deviceId;
      state.devices.set(id, {...state.devices.get(id), deviceId: id, deviceName: state.native.deviceName,
        volume: state.native.volume, muted: state.native.muted, online: true, clientAt: Date.now()});
      renderDevices();
  ...
  function bindEvents() {
    ...
    dom.down?.addEventListener('click', () => stepVolume(-1));
    dom.up?.addEventListener('click', () => stepVolume(1));
